    ...
    for (int i = 0; i < NO_OF_CUSTOMERS; i++) {
        ...
        if (pid == 0) {   // If child process
            ...
            if (scenario == 0) {
                ...
                order_product(i, random_Product_ID, random_Quantity);
            } else {
                ...
                order_products(i, order_Data, ordered_Product_Count);
                free(order_Data);
            }
            exit(0);
        }
...
int main() {
    ...
    const int protection_Flags = (PROT_WRITE | PROT_READ);
    const int map_Flags = (MAP_SHARED | MAP_ANON); 
    customer_Arr = (Customer*) mmap(NULL, sizeof(Customer) * NO_OF_CUSTOMERS, protection_Flags, map_Flags, -1, 0);
    product_Arr = (Product*) mmap(NULL, sizeof(Product) * NO_OF_PRODUCTS, protection_Flags, map_Flags, -1, 0);
	...
	pthread_mutexattr_setpshared(&mutex_Attributes, PTHREAD_PROCESS_SHARED);    // Create process attribute which allows to share mutex lock across different processes
    printf("Product ID\tQuantity\tPrice\n");
    for (int i = 0; i < NO_OF_PRODUCTS; i++) {  // Initialize products
        pthread_mutex_init(&product_Arr[i].mutex, &mutex_Attributes);
    ...
    while(wait(NULL) > 0);  // Wait until all child processes complete
